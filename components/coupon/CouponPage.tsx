...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`text-[12px] font-bold px-2.5 py-0.5 rounded-full border ${s.cls}`}>
      {s.label}
    </span>
  );
}
...
function TypeBadge({ type, label }: { type: string; label: string }) {
  ...
  return (
    <span className={`text-[12px] font-bold px-2.5 py-0.5 rounded ${colors[type] ?? "bg-gray-100 text-gray-600"}`}>
      {label}
    </span>
  );
}
      ...
      <div className="px-4 pt-3 pb-4">
        ...
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex-1 min-w-0">
            <p className="text-[17px] font-bold text-ap-primary leading-tight">{item.name}</p>
          ...
          <div className="text-right flex-shrink-0">
            <p className="text-[28px] font-extrabold text-amber-500 tabular-nums leading-tight">
        ...
        <div className="bg-ap-bg rounded-xl px-3 py-2 mb-3 flex items-center justify-between">
          <span className="text-[13px] text-ap-secondary font-bold">CODE</span>
          <span className="text-[17px] font-bold font-mono tracking-widest text-ap-primary">{item.code}</span>
        </div>

        {/* Meta info */}
        <div className="grid grid-cols-3 gap-2 mb-3 text-center">
          <div className="bg-ap-bg/60 rounded-xl py-2.5">
            <p className="text-[12px] font-semibold text-ap-secondary">{t.labelTurnover}</p>
            <p className="text-[15px] font-bold text-ap-primary tabular-nums">×{item.turnpro}</p>
          </div>
          <div className="bg-ap-bg/60 rounded-xl py-2.5">
            <p className="text-[12px] font-semibold text-ap-secondary">{t.labelLimit}</p>
            <p className="text-[15px] font-bold text-ap-primary tabular-nums">{item.amount_limit}</p>
          </div>
          <div className="bg-ap-bg/60 rounded-xl py-2.5">
            <p className="text-[12px] font-semibold text-ap-secondary">{t.labelExpire}</p>
            <p className="text-[13px] font-bold text-ap-primary">
              {item.date_expire
        ...
        {item.can_claim && !isClaimed ? (
          <button
            ...
            onClick={handleClaim}
            disabled={pending}
            className="w-full py-3 rounded-xl bg-gradient-to-r from-amber-400 to-orange-500 text-white text-[15px] font-bold hover:opacity-90 active:scale-95 transition-all disabled:opacity-60 disabled:cursor-not-allowed shadow-sm"
          >
            {pending ? t.claiming : t.claimBtn}
          </button>
        ) : isClaimed ? (
          <div className="w-full py-3 rounded-xl bg-ap-bg border border-ap-border text-center text-[15px] font-bold text-ap-secondary">
            ✓ {item.status_label}
          </div>
        ) : null}
        ...
        {/* Toast */}
        {toast && (
          <div className={`mt-2 px-3 py-2 rounded-xl text-[13px] font-semibold text-center ${toast.ok ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-600"}`}>
            {toast.msg}
          </div>
        )}
      </div>
...
function RedeemBox({ onRedeem }: { onRedeem: (code: string) => Promise<{ success: boolean; message?: string }> }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-ap-border shadow-card p-4">
      <p className="text-[14px] font-bold text-ap-secondary uppercase tracking-wide mb-3">🎟 {t.redeemLabel}</p>
      <form onSubmit={handleRedeem} className="flex gap-2">
        <input
          ...
          maxLength={30}
          className="flex-1 border border-ap-border rounded-xl px-3 py-3 text-[15px] font-mono font-bold uppercase tracking-wider outline-none focus:border-ap-blue bg-white transition-all"
        />
        <button
          type="submit"
          disabled={!code.trim() || pending}
          className="px-5 py-3 rounded-xl bg-ap-blue text-white text-[15px] font-bold hover:bg-ap-blue-h active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {pending ? "..." : t.redeemBtn}
        </button>
      </form>
      {toast && (
        <div className={`mt-2 px-3 py-2 rounded-xl text-[13px] font-semibold ${toast.ok ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-600"}`}>
          {toast.msg}
        </div>
      )}
    </div>
      ...
      <div className="max-w-2xl mx-auto px-4 pt-5 space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[14px] font-semibold text-ap-secondary">{t.count.replace("{n}", String(count))}</p>
          </div>
        </div>
        ...
        {items.length === 0 ? (
          <div className="bg-white rounded-2xl border border-ap-border shadow-card py-16 text-center">
            ...
            <p className="text-[17px] font-bold text-ap-primary">{t.empty}</p>
            <p className="text-[14px] font-semibold text-ap-secondary mt-1">{t.emptyDesc}</p>
          </div>
        ) : (
