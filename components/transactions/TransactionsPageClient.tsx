...
function DirectionBadge({ direction, t }: { direction: TxRow["direction"]; t: TT }) {
  return direction === "CREDIT"
    ? <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-500/10 text-emerald-600">{t.credit}</span>
    : <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-red-400/10 text-red-500">{t.debit}</span>;
}
    ...
    <div className="flex items-start justify-between px-4 py-3.5 border-b border-ap-border last:border-0">
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5 flex-wrap">
          <p className="text-[13px] font-semibold text-ap-primary">{tx.title}</p>
          <DirectionBadge direction={tx.direction} t={t} />
        </div>
        {tx.detail && (
          <p className="text-[11px] text-ap-secondary mt-0.5 leading-relaxed">{tx.detail}</p>
        )}
        <p className="text-[11px] text-ap-tertiary mt-0.5">{tx.date}</p>
        <div className="mt-1"><StatusBadge status={tx.status} t={t} /></div>
      </div>
      <div className="text-right flex-shrink-0 pl-4">
        <p className={`text-[16px] font-bold tabular-nums ${isCredit ? "text-emerald-500" : "text-red-500"}`}>
          {isCredit ? "+" : ""}{formatNumber(tx.signedAmount, locale)}
        </p>
        {tx.balanceAfter > 0 && (
          <p className="text-[11px] text-ap-tertiary tabular-nums mt-0.5">
  ...
  return (
    <div className="bg-white rounded-2xl border border-ap-border shadow-card px-4 py-3 grid grid-cols-2 sm:grid-cols-4 gap-3">
      <div className="text-center">
        <p className="text-[11px] text-ap-tertiary font-medium">{t.summaryTotal}</p>
        <p className="text-[15px] font-bold text-ap-primary tabular-nums">{summary.count}</p>
      </div>
      <div className="text-center">
        <p className="text-[11px] text-ap-tertiary font-medium">{t.summaryCredit}</p>
        <p className="text-[15px] font-bold text-emerald-600 tabular-nums">+{formatNumber(summary.totalCredit, locale)}</p>
      </div>
      <div className="text-center">
        <p className="text-[11px] text-ap-tertiary font-medium">{t.summaryDebit}</p>
        <p className="text-[15px] font-bold text-red-500 tabular-nums">-{formatNumber(summary.totalDebit, locale)}</p>
      </div>
      <div className="text-center">
        <p className="text-[11px] text-ap-tertiary font-medium">{t.summaryNet}</p>
        <p className={`text-[15px] font-bold tabular-nums ${netPositive ? "text-emerald-600" : "text-red-500"}`}>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
          {TAB_IDS.map((id) => (
            <button key={id} type="button"
              ...
              className={[
                "flex items-center justify-center gap-1.5 px-2 py-2 rounded-xl text-[12px] font-semibold transition-colors border",
                id === tabId
