...
function AppIcon({ name, className }: { name: string; className?: string }) {
  ...
  const emoji = map[name as IconName] ?? name;
  return (
...
export default function Navbar({ logoUrl, balance, diamond, userName, userPhone, mobileNavItems }: NavbarProps) {
  ...
  const navLinks: NavLink[] = mobileNavItems && mobileNavItems.length > 0
        ...
        <div className="grid w-full" style={{ gridTemplateColumns: `repeat(${navLinks.length}, 1fr)` }}>
          {navLinks.map((l) => {
            ...
            return (
              <Link key={l.href} href={l.href}
                className={[
                  "flex flex-col items-center justify-center gap-0.5 min-w-0 py-1.5 transition-all active:scale-95",
