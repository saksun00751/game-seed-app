...
import type { TxRow } from "@/lib/server/transactions";
...
export default async function TransactionsPage({ params, searchParams }: Props) {
  const [{ locale }, user, sp] = await Promise.all([
    params ?? Promise.resolve({ locale: "th" }),
    requireAuth(),
    searchParams,
  ]);
  const tabId  = TABS.find((t) => t.id === sp?.tab) ? sp!.tab! : "deposit";
  const tab    = TABS.find((t) => t.id === tabId)!;

  const rows = await getTransactionsByTab(user.id, tabId);
      ...
      <div className="max-w-5xl mx-auto px-4 pt-5 space-y-4">
        ...
        <div className="-mx-4 px-4 flex gap-2 overflow-x-scroll pb-1 [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: "none" }}>
          {TABS.map((t) => (
            <Link
              key={t.id}
              href={`/${locale}/transactions?tab=${t.id}`}
              className={[
