...
import type { BetRateRow } from "@/lib/types/bet";
...
interface Props {
  ...
  searchParams?: Promise<{ lottery?: string; draw_id?: string }>;
}
...
function CategoryCard({
  ...
  return (
    ...
    >
      ...
      <div>
        <div className="font-bold tracking-tight leading-tight text-[15px] text-white">{cat.label}</div>
...
export default async function BetRoute({ params, searchParams }: Props) {
  ...
  if (lottery) {
    ...
    const { drawId, numberLimits, betRates, myBetHistory, pastResults } = await getBetPageData(lottery, token, lang);
